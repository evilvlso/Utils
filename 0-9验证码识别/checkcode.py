...
def compare(image):
    ...
    result = 0
    for i in range(0, 9):
        image_com = cv2.imread('D:\\wenshusan\\wenshusan\\tools\\train\\{0}.jpg'.format(i))
...
def distinguish(filename):
    ...
    for item in arr:
        ...
        cv2.imwrite('D:\\wenshusan\\wenshusan\\verifitioncode\\split.jpg', res)
        res = cv2.imread('D:\\wenshusan\\wenshusan\\verifitioncode\\split.jpg')
        check_result += str(compare(res))
    return check_result

if __name__ == '__main__':
    a = distinguish('../../checkCode.jpg')
    print(a)